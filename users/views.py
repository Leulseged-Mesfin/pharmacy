...
class UserListCreateAPIView(APIView):
    ...
    def get(self, request, format=None):
        try:
            user = request.user
            if not (user.role == 'Doctor' or user.is_superuser == True):
                return Response(
            ...
            if user.groups.filter(name='Doctor').exists():
                # Exclude superusers for Doctors
                users = User.objects.filter(is_superuser=False)
            else:
                # Retrieve all users for non-Doctors and non-salesman
    ...
    def post(self, request):
        try:
            user = request.user
            if not (user.role == 'Doctor' or user.is_superuser):
                return Response(
