...
from .models import Product, Supplier, Order, OrderItem, Category, PatientInfo
...
class ProductListCreateAPIView(APIView):
    ...
    def get(self, request, format=None):
        try:
            user = request.user
            if not (user.role == 'Doctor' or user.is_superuser == True or user.role == 'Pharmacist'):
                return Response(
...
class PatientListCreateAPIView(APIView):
    ...
    def get(self, request, format=None):
        try:
            ...
            if not (user.role == 'Doctor' or user.is_superuser == True or user.role == 'Pharmacist'):
                return Response(
                    {"error": "You are not authorized to retrive the patients."},
                    status=status.HTTP_403_FORBIDDEN
                ) 
            ...
            return Response(
                {"error": f"An error occurred while Retriving the patients.  {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def post(self, request, format=None):
        try:
            ...
            if not (user.role == 'Doctor' or user.is_superuser == True or user.role == 'Pharmacist'):
                return Response(
                    {"error": "You are not authorized to create the patient."},
                    status=status.HTTP_403_FORBIDDEN
                )               
            ...
            validated_data = serializer.validated_data
            serializer.create(validated_data)
            return Response({"message": f"patient Created successfully."}, status=status.HTTP_201_CREATED)     
        except KeyError as e:
            return Response(
                {"error": f"An error occurred while creating the patient.  {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class PatientRetrieveUpdateDeleteAPIView(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            if not (user.role == 'Doctor' or user.is_superuser == True or user.role == 'Pharmacist'):
                return Response(
                    {"error": "You are not authorized to retrive the patient."},
                    status=status.HTTP_403_FORBIDDEN
                )                
            if not PatientInfo.objects.filter(id=pk).exists():
                return Response(
            ...
            return Response(
                {"error": f"An error occurred while Retriving the patient.  {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def put(self, request, pk):
        try:
            ...
            if not (user.role == 'Doctor' or user.is_superuser == True or user.role == 'Pharmacist'):
                return Response(
                    {"error": "You are not authorized to update the patient."},
                    status=status.HTTP_403_FORBIDDEN
                )                
            if not PatientInfo.objects.filter(id=pk).exists():
                return Response(
            ...
            if not serializer.is_valid():
                return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
            validated_data = serializer.validated_data
            serializer.update(patient, validated_data)
            return Response({"message": f"patient Updated successfully."}, status=status.HTTP_200_OK)      
        except KeyError as e:
            return Response(
                {"error": f"An error occurred while updating the patient.  {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def delete(self, request, pk):
        try:
            ...
            if not (user.role == 'Doctor' or user.is_superuser == True or user.role == 'Pharmacist'):
                return Response(
                    {"error": "You are not authorized to delete the patient."},
                    status=status.HTTP_403_FORBIDDEN
                )               
            if not PatientInfo.objects.filter(id=pk).exists():
                return Response(
            ...
            PatientInfo.objects.get(id=pk).delete()
            if not PatientInfo.objects.filter(id=pk).exists():
                return Response({"message": f"patient Deleted successfully."},
                    status=status.HTTP_204_NO_CONTENT
                )
            else:
                return Response(
